...
from job_watcher.search.bocha_provider import BochaWebSearchProvider  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Bocha web search tasks.")
...
def main() -> int:
    args = parse_args()
    provider = BochaWebSearchProvider(timeout=args.timeout)
    if args.query:
    ...
    tasks = tasks[tasks["provider"] == "bocha"].head(args.max_tasks)
    total = 0
